...
class Progress(commands.Cog):
    ...
    async def progress(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        print(f"🔍 Fetching progress for User: {target_user.display_name} (ID: {user_id})")

        progress_data = await db.get_progress(user_id)
        if not progress_data:
            print(f"❌ No progress data found for {user_id}")
        ...
        print(f"📈 Total Gym Check-ins: {total_gym_checkins}, Total Food Logs: {total_food_logs}")

        # Fetch first & most recent weight change
        first_weight, recent_weight, weight_change = await db.get_weight_change(user_id)
        print(
            f"📊 Retrieved weight change for {user_id}: First={first_weight}, Recent={recent_weight}, Change={weight_change}")
        ...
        if first_weight is not None and recent_weight is not None:
            ...
            print(f"❌ No valid weight change data for {user_id}")
        ...
        print(f"💪 PR Data Retrieved: {pr_data}")
        ...
        print(f"🏋️‍♂️ PR Values - Deadlift: {deadlift}, Bench: {bench}, Squat: {squat}")
        ...
        print(f"📤 Sending response for {target_user.display_name}...")
